...
static void fill_controls(ControlList *ctrls, const parameters_t *params) {
    ...
    } else if (strcmp(params->af_mode, "auto") == 0) {
    ...
    int af_range;
    if (strcmp(params->af_range, "normal") == 0) {
        af_range = controls::AfRangeNormal;
    } else if (strcmp(params->af_range, "macro") == 0) {
    ...
    } else if (strcmp(params->af_range, "fast") == 0) {
    ...
    ctrls->set(controls::LensPosition, params->lens_position);
...
bool camera_start(camera_t *cam) {
    ...
    fill_controls(camp->ctrls.get(), camp->params);
...
void camera_reload_params(camera_t *cam, const parameters_t *params) {
    ...
    std::lock_guard<std::mutex> lock(camp->ctrls_mutex);
    fill_controls(camp->ctrls.get(), params);
}
